...
def callback_feedback(data):
    ...
    print ('vel of car:', str(data.twist.twist.linear.x), str(data.twist.twist.linear.y),str(data.twist.twist.linear.z))
    print ('c')
    ...
    cp1 = cp
    cross2 = [(x_bot - data1.poses[cp1].pose.position.x), (y_bot - data1.poses[cp1].pose.position.y)]
    ...
    while Lf > L and (cp + 1) < len(x_p.poses):
        dx = data1.poses[cp + 1].pose.position.x - data1.poses[cp].pose.position.x
        dy = data1.poses[cp + 1].pose.position.y - data1.poses[cp].pose.position.y
