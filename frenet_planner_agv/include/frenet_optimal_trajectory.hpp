...
{
	private :
		vecD t, d, d_d, d_dd, d_ddd, s, s_d, s_dd, s_ddd, x, y, yaw, ds, c;//ye c kya hain radius of curvature hain kya, agr haa to frenet_ros main iska use na krke abhi local variable ka use ho rha hain initial conditions() main
	...
	*/
	public :
		...
		vecD get_c();
		double get_cf();
		// void add_x(double ); //are these used anywhere?
		// void add_y(double ); 
		 //ye c ky ahian
		void set_Jp(double);
		void set_Js(double);
