...
# Scaling of velocity vectors -> 1 cell = 1/SCALE_VELOCITY_VEC m/s
SCALE_VELOCITY_VEC = 0.4
# Define, how many cells your grid should contain in each direction.
GRID_X_MIN = -2
GRID_X_MAX = 2
GRID_Y_MIN = -2
GRID_Y_MAX = 1
...
def arrowMarker(color='green', orientation='auto'):
    ...
    arrow.add(svg.polyline(
        [(0, 0), (10, 5), (0, 10), (1, 5)], fill=color, opacity=1.0))
    svg.defs.add(arrow)
    return arrow
...
def quaternion2YawDegree(orientation):
    ...
    @rtype:  float  
    """
    yawRad = math.atan2(2*(orientation.x*orientation.y+orientation.z *
                        orientation.w), 1-2*(pow(orientation.y, 2)+pow(orientation.z, 2)))
...
if __name__ == '__main__':
    ...
    svg.viewbox(GRID_X_MIN*SCALE-1*SCALE, GRID_Y_MIN*SCALE-1*SCALE, GRID_X_MAX *
                SCALE-GRID_X_MIN*SCALE+2*SCALE, GRID_Y_MAX*SCALE-GRID_Y_MIN*SCALE+2*SCALE)
    ...
    for y in range(GRID_Y_MAX):
        hLines.add(svg.line(start=(GRID_X_MIN*SCALE, SCALE+y*SCALE),
    ...
    for x in range(GRID_X_MAX):
        vLines.add(svg.line(start=(SCALE+x*SCALE, GRID_Y_MIN*SCALE),
    ...
    legend.translate(tx=GRID_X_MIN*SCALE, ty=GRID_Y_MAX*SCALE + 30)
    svg.add(legend)
    ...
    while not rospy.is_shutdown():
        if feedbackMsg:
            # Leave loop after receiving all necessary TEB information (see callbacks) to finish drawing
            break
        rate.sleep()
    # ------------------------------------------------------------------
    ...
    for index, traj in enumerate(feedbackMsg.trajectories):
        ...
        # color
        traj_color = svgwrite.rgb(random.randint(0, 255), random.randint(
        ...
        for point in traj.trajectory:
            ...
            points.append((point.pose.position.x*SCALE, -
    ...
    goal_pose = feedbackMsg.trajectories[0].trajectory[len(
        feedbackMsg.trajectories[0].trajectory)-1].pose
    start_position = start_pose.position
    goal_position = goal_pose.position
    svg.add(svg.circle(center=(start_position.x*SCALE, -start_position.y *
            SCALE), r=10, stroke_width=1, stroke='blue', fill='blue'))
    svg.add(svg.text("Start", (start_position.x*SCALE-70, -
            start_position.y*SCALE+45), font_size=35))  # Add label
    svg.add(svg.circle(center=(goal_position.x*SCALE, -goal_position.y *
            SCALE), r=10, stroke_width=1, stroke='red', fill='red'))
    svg.add(svg.text("Goal", (goal_position.x*SCALE-40, -
            goal_position.y*SCALE+45), font_size=35))  # Add label

    # draw start arrow
    start_arrow = svg.polyline(
        [(0, -1), (6, -1), (5, -5), (15, 0), (5, 5), (6, 1), (0, 1)], fill='blue', opacity=1.0)
    ...
    goal_arrow = svg.polyline(
        [(0, -1), (6, -1), (5, -5), (15, 0), (5, 5), (6, 1), (0, 1)], fill='red', opacity=1.0)
    ...
    for obstacle in feedbackMsg.obstacles:
        if len(obstacle.polygon.points) == 1:  # point obstacle
            ...
            svg.add(svg.circle(center=(point.x*SCALE, -point.y*SCALE), r=OBSTACLE_DIST,
                    stroke_width=1, stroke='grey', fill='grey', opacity=0.3))
            svg.add(svg.circle(center=(point.x*SCALE, -point.y*SCALE),
                    r=15, stroke_width=1, stroke='black', fill='black'))
            svg.add(svg.text("Obstacle", (point.x*SCALE-70, -
                    point.y*SCALE+45), font_size=35))  # Add label
        if len(obstacle.polygon.points) == 2:  # line obstacle
            line_start = obstacle.polygon.points[0]
            line_end = obstacle.polygon.points[1]
            svg.add(svg.line(start=(line_start.x*SCALE, -line_start.y*SCALE), end=(line_end.x *
                    SCALE, -line_end.y*SCALE), stroke='black', fill='gray', stroke_width=1, opacity=1.0))
            svg.add(svg.text("Obstacle", (line_start.x*SCALE-70, -
                    line_start.y*SCALE+45), font_size=35))  # Add label
        if len(obstacle.polygon.points) > 2:  # polygon obstacle
            ...
            svg.add(svg.polygon(points=vertices, stroke='black',
                    fill='gray', stroke_width=1, opacity=1.0))
            svg.add(svg.text(
                "Obstacle", (obstacle.polygon.points[0].x*SCALE-70, -obstacle.polygon.points.y*SCALE+45), font_size=35))  # Add label
